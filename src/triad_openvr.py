...
class vr_tracked_device():
    ...
    def get_pose_euler(self):
        pose = self.vr.getDeviceToAbsoluteTrackingPose(openvr.TrackingUniverseStanding, 0,openvr.k_unMaxTrackedDeviceCount)
        #deviceIndexNum = self.index
        #res = convert_to_euler(pose[self.index].mDeviceToAbsoluteTracking)
        #res.append(deviceIndexNum)
        return convert_to_euler(pose[self.index].mDeviceToAbsoluteTracking)#res
...
class triad_openvr():
    ...
    def get_state(self,controller_id):
        pControllerState = self.vr.getControllerState(controller_id)
